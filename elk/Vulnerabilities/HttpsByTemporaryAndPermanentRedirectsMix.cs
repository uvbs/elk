namespace Elk.Vulnerabilities
{
  ...
  public class HttpsByTemporaryAndPermanentRedirectsMix : IVulnerabilityDefinition
  {
    ...
    public HttpsByTemporaryAndPermanentRedirectsMix()
    {
    ...
    public bool IsTargetVulnerable(DataForVulnerabilityScanner scannerData)
    {
      ...
      if (numberOfRedirectElements > 0)
      {
        ...
        for (int i = 0; i < numberOfRedirectElements; i++)
        {
          ...
          else if (scannerData.ResponseEntityChain[i].ResponseStatusCode == 302 ||
